...
class TurningAnt(env.Env):
  ...
  * v0: Initial versions release (1.0.0)
  ...
  def get_us(self, obs):
      # obs are qpos + qvel + cfrc + [qp.pos[self._hidden_idx]]...
      return obs[..., -1]
  ...
  def reset(self, rng: jp.ndarray) -> env.State:
    ...
    pos = jp.index_update(qp.pos, self._hidden_idx, hidden)
  ...
  def step(self, state: env.State, action: jp.ndarray) -> env.State:
    ...
    hidden = state.qp.pos[self._hidden_idx][0]

    velocity = (qp.pos[0] - state.qp.pos[0]) / self.sys.config.dt
    forward_reward = hidden * velocity[0] + (1 - hidden) * velocity[1]  # h * vel_x + (1 - h) * vel_y
  ...
  def _get_obs(self, qp: brax.QP, info: brax.Info) -> jp.ndarray:
    ...
    return jp.concatenate(qpos + qvel + cfrc + [qp.pos[self._hidden_idx]])
